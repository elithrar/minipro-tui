...
export function dangerousOptionWarnings(options: AdvancedOptions): string[] {
  ...
  if (options.skipVerify) warnings.push(`Explicit minipro verify is disabled.${options.disableReadbackCompare ? " No post-write byte comparison will run." : " Independent readback byte comparison remains enabled."}`);
